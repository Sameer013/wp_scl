 ...
 */
function academic_pro_sanitize_select( $input, $setting ) {
 ...
 */
function academic_pro_sanitize_image( $image, $setting ) {
	/*
 ...
 *                    the setting default.
 */
function academic_pro_sanitize_number_range( $number, $setting ) {
...
* @since Academic Pro 1.0
*/
function academic_pro_sanitize_page( $input ) {
 ...
 */
function academic_pro_sanitize_checkbox( $checked ) {
 ...
 * @since Academic Pro 1.0
 */
function academic_pro_sanitize_category_list( $input ) {
	if ( $input != '' ) {
		$args = array(
						'type'			=> 'post',
						'child_of'      => 0,
 ...
 */
function academic_pro_santize_allow_tag( $input, $setting ) {
 ...
 * @return string The input with only allowed tag i.e. anchor
 */
function academic_pro_santize_line_break( $input ) {
	$input = wp_kses( $input, array(
		'p'  => array(),
		'br' => array(),
		) );
...
/**
 * Sanitizes testimonial post list
 * @param  $input entered value
 * @return sanitized output
 *
 */
function academic_pro_sanitize_testimonial_list( $input ) {
	if ( $input != '' ) { 
		$args = array(
		...
		$testimonial_post_list =	array();
		
		foreach ( $testimonial_posts as $post )
			$testimonial_post_list     =	array_merge( $testimonial_post_list, array( $post->ID ) );

		if ( count( array_intersect( $input, $testimonial_post_list ) ) == count( $input ) ) {
	    	return $input;
	    } 
	    else {
    		return '';
